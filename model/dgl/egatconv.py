...
class EGATConv(nn.Module):
    ...
    def __init__(self,
        ...
        if residual:
            if self._in_dst_node_feats != out_node_feats * num_heads:
                self.res_fc_node = nn.Linear(
                    self._in_dst_node_feats, num_heads * out_node_feats, bias=False)
            else:
                self.res_fc_node = nn.Sequential()

            if self._in_edge_feats != out_edge_feats * num_heads:
                self.res_fc_edge = nn.Linear(
                    self._in_edge_feats, num_heads * out_edge_feats, bias=False)
            else:
                self.res_fc_edge = nn.Sequential()
        else:
            self.register_buffer('res_fc_node', None)
            self.register_buffer('res_fc_edge', None)
    ...
    def reset_parameters(self):
        ...
        if isinstance(self.res_fc_node, nn.Linear):
            nn.init.xavier_normal_(self.res_fc_node.weight, gain=gain)
        if isinstance(self.res_fc_edge, nn.Linear):
            nn.init.xavier_normal_(self.res_fc_edge.weight, gain=gain)
    ...
    def forward(self, graph, nfeats, efeats, get_attention=False):
        ...
        with graph.local_scope():
            ...
            # residual
            if self.res_fc_node is not None:
                # Use -1 rather than self._num_heads to handle broadcasting
                dst_prefix_shape = nfeats_dst.shape[:-1]
                resval = self.res_fc_node(nfeats_dst).view(*dst_prefix_shape, -1, self._out_node_feats)
                h_out = h_out + resval
            if self.res_fc_edge is not None:
                edge_prefix_shape = efeats.shape[:-1]
                resval = self.res_fc_edge(efeats).view(*edge_prefix_shape, -1, self._out_edge_feats)
                f_out = f_out + resval
